  ...
  <div id="categories">
    <div class="category-buttons">
      <button
        ...
        :class="[
          'category-btn',
          { 'selected-btn': category === this.category },
        ]"
      >
        <img
...
export default {
  ...
  computed: {
    ...
    filteredDealsByCategory() {
      // console.log("Filtering deals with searchQuery:", this.searchQuery);
      // console.log("Filtering deals with category:", this.category);
      return this.deals
        .filter((deal) => deal.category === this.category)
        .filter((deal) => {
  ...
  methods: {
    ...
    sortDeals(criteria) {
      if (criteria === "name") {
        ...
        this.deals.sort((a, b) => {
          const distanceA = parseFloat(a.distance) || 100000; // Ensure it's a number
          const distanceB = parseFloat(b.distance) || 100000; // Ensure it's a number
          return distanceA - distanceB; // Sort numerically
        });
      } else if (criteria === "expiry") {
        this.deals.sort((a, b) => {
          const expiryA = new Date(a.validUntil.seconds) || 0; // Default to 0 if undefined
          const expiryB = new Date(b.validUntil.seconds) || 0; // Default to 0 if undefined
